...
public class Paging {
	...
	private PageVO vo; //page(현재 페이지), perPageNum(페이지 당 보여질 게시물의 수), 검색분류, 검색어
	
	public Paging(PageVO vo) {
	  this.vo=vo;
	}
	...
	public String makeQuery(int page) {
		UriComponentsBuilder uriComponentsBuilder = UriComponentsBuilder.newInstance()
				...
				.queryParam("perPageNum", this.vo.getPerPageNum());
				
			if (this.vo.getSearchType() != null) {
				uriComponentsBuilder
					.queryParam("searchType", this.vo.getSearchType())
					.queryParam("keyword", this.vo.getKeyword());
			}
	...
	//startPage, endPage, prev, next 를 계산
		public void calcData() {
			int page = this.vo.getPage();
			int perPageNum = this.vo.getPerPageNum();
